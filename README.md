...
2. Lag en lokal `.env` med minst `DATABASE_URL`, `JWT_SECRET` og `ADMIN_PASSWORD`
...
- `DATABASE_URL`
  - Postgres-tilkobling, f.eks. fra Neon
...
- sett `DATABASE_URL`, `JWT_SECRET`, `ADMIN_PASSWORD` og relevante SMS-variabler i Vercel før deploy
